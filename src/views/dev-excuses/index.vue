...
<template>
  <div class="min-h-screen bg-bg-deep text-text-primary font-body flex flex-col items-center justify-center p-6 relative overflow-hidden">
    
    <!-- Background Number -->
    <span class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-display text-[15rem] md:text-[25rem] font-bold text-accent-amber/5 select-none pointer-events-none -z-10 animate-fade-up">
      404
    </span>

    <div class="w-full max-w-2xl relative animate-fade-up animate-delay-1">
      <!-- Issue Badge -->
      <div class="absolute -top-4 -right-4 md:-right-8 bg-accent-coral text-bg-deep font-display font-bold text-xs tracking-widest px-3 py-1.5 rotate-3 z-10 shadow-lg select-none">
        VOL.ERR / 2026
      </div>

      <!-- Main Card -->
      <div class="border border-border-default bg-bg-surface p-8 md:p-12 transition-all duration-300 hover:-translate-y-1 hover:border-accent-coral hover:shadow-lg hover:shadow-accent-coral/5 flex flex-col items-center">
        
        <h1 class="font-display text-2xl md:text-3xl font-semibold text-text-primary mb-8 w-full flex items-center justify-center md:justify-start gap-3">
          <span class="text-accent-coral font-display text-sm tracking-widest">//</span>
          Dev's Excuse Generator
        </h1>

        <!-- The Terminal / Display Area -->
        <div class="w-full bg-bg-deep border border-border-default p-6 min-h-[180px] flex items-center justify-center text-center relative group">
          <p class="font-display text-xl md:text-2xl text-accent-amber tracking-wide leading-relaxed">
            <span class="opacity-50 mr-2">&gt;</span>
            {{ currentExcuse }}
            <span v-if="isGenerating" class="inline-block w-2.5 h-6 bg-accent-amber animate-pulse ml-1 align-middle"></span>
          </p>
          ...
          <button 
            ...
            :class="{ 
              'text-text-secondary border border-border-default hover:text-accent-coral hover:border-accent-coral opacity-0 group-hover:opacity-100': !copied, 
